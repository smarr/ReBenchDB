...
```{r suites, results='asis', echo=FALSE, dev='svg', fig.keep='all', fig.height=0.4, fig.width=3.5, timeit=TRUE}
    ...
    for (b in levels(data_s$bench)) { data_b <- data_s %>% filter(bench == b) %>% droplevels()
      ...
      for (ea in levels(data_i$extraargs)) { data_ea <- data_i %>% filter(extraargs == ea) %>% droplevels()
        ...
        if (nrow(stats_b) > 0) {
          ...
          cp('<td><span class="stats-median" title="median">', r2(stats_b$median), '</span></td>')
          cp('<td><span class="stats-change" title="change over median">', pro(stats_b$change_m), '</span></td>')
          cp('<td><button type="button" class="btn btn-sm" data-toggle="popover" data-content="<code>', cmdline, '</code>">',
             '</button></td>');
          cp('</tr>')
        } else {
          cp('<tr>')
